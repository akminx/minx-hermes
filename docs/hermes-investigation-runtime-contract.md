...
2. Choose read tools from the programmatic allowlist in `hermes_loop/runtime.py`. `docs/minx-investigation-tool-catalog.md` documents that same shipped allowlist and notes which broader product tools are intentionally outside the runner.
...
5. Stop at the configured budget. The runtime default is 12 domain tool calls or 120 seconds; `scripts/minx-investigate.py` defaults to a smaller CLI budget unless flags override it.
...
The model is not part of the storage contract. The production runner uses an OpenAI-compatible chat endpoint. Use `MINX_INVESTIGATION_MODEL` to choose the deployment model; `google/gemini-2.5-flash` is the recommended OpenRouter example for current smokes.
